...
### 2026-08-17 · 휴직 기간 구분 (`absence_length`)

휴직이 정원(가능인원)을 차지하는지가 기간에 따라 갈린다.

- **3개월 이상** — 부대 정원에서 아예 빠지므로 가능인원이 줄지 않는다.
- **3개월 미만** — 자리는 그대로 남아 있어 그 기간 내내 한 자리를 차지한다.

예) 출동율 80%로 가능인원이 3명일 때, 3개월 이상 휴직자가 있어도 3명이 갈 수 있지만,
3개월 미만 휴직자가 있으면 실질적으로 2명만 갈 수 있다.

**이 SQL을 먼저 실행한 뒤에 배포해야 한다.** 순서를 바꾸면 휴직 신청만 없는 칸을 찾다가
실패한다(다른 유형은 이 칸을 쓰지 않아 멀쩡하다).

```sql
-- 휴직 기간 구분. 휴직이 아닌 신청에서는 비어 있다.
ALTER TABLE leave_requests ADD COLUMN IF NOT EXISTS absence_length text;
```

> `type`처럼 허용 목록(check 제약조건)을 걸지 않았다. 걸어두면 나중에 구분을 하나 더
> 늘릴 때마다 SQL을 다시 실행해야 하고, 잊으면 `특가` 때처럼 신청이 조용히 거부된다.

값이 비어 있는 휴직(이 기능 이전에 등록된 것)은 지금까지와 똑같이 한 자리를 차지한다.
판정은 `lib/utils.ts`의 `occupiesQuota`가 전담하므로 세는 규칙을 바꿀 때는 그 함수만 고치면 된다.
...
### 2026-08-08 · 서무 계정 이름 변경

`관리자 admin` → **`2제대 서무`** (`name='2제대'`, `rank='서무'`).

**이름과 계급이 곧 로그인 아이디이므로 로그인 아이디가 바뀐 것이다.** 비밀번호는 그대로다.
나중에 세 제대가 함께 쓰게 되면 `1제대 서무` · `3제대 서무`로 나란히 이어진다
(`multi-unit-plan.md` 참고).

바꾸기 전에 확인한 것: 같은 이름+계급이 없을 것, 전체 26명 중 이름+계급 중복이 없을 것
...
2026-09-17 인사이동 뒤로 **일근 → 당직 → 비번**이 3일마다 돌아간다(기준일 **2026-09-19 = 일근**,
`lib/utils.ts`의 `DUTY_CYCLE_ANCHOR`). 우리 부대가 일근에 걸리는 날은 사람이 더 남아 있어
연가를 한 명 더 보낼 수 있다 — 가능인원 2명·총 한도 3명이 그 날만 3명·4명이 된다.

늘어나는 인원을 **코드에 고정하지 않고 정원 설정마다 두었다.** 출동율이 바뀌면 추가 인원도
달라질 수 있고, 그때마다 배포하지 않고 서무가 그 자리에서 고칠 수 있어야 하기 때문이다.

**이 SQL을 먼저 실행한 뒤에 배포해야 한다.** 순서를 바꾸면 정원 설정 저장이 없는 칸을 찾다가
실패한다(달력 표시는 칸이 없어도 1명으로 보고 그대로 동작한다).

```sql
-- 우리 부대가 일근인 날에만 가능인원과 총 한도에 함께 더해지는 인원.
-- 기존 설정도 일근일에는 한 명 더 보내던 기준이므로 기본값을 1로 둔다.
ALTER TABLE quota_settings ADD COLUMN IF NOT EXISTS dayduty_bonus integer NOT NULL DEFAULT 1;
```

기준일(2026-09-19)보다 앞선 날짜는 인사이동 전이라 편성이 달랐으므로 달력에 아무 표시도 하지
않고 정원도 보정하지 않는다. 그래서 `2026-08-07 ~ 2026-09-04` 설정은 이 칸이 1이 되어도
영향을 받지 않는다.
