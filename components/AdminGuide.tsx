        ...
        <div className="px-6 pb-6 pt-2 border-t border-gray-200 space-y-6 text-sm text-gray-700">
          <Section title="이 페이지에서 하는 일">
            <ul className="space-y-1 list-disc list-inside">
              ...
              <li>
                <B>사용 현황</B> — 그 달에 연가를 쓴 대원 전원의 순위를 봅니다.
              </li>
              <li>
                <B>대리 입력</B> — 대원 대신 연가·병가를 <B>신청·수정·취소</B>합니다.
              </li>
            </ul>
            <p className="mt-2">
              본인 연가 신청과 <B>순번 지정</B>은 이 페이지가 아니라 <B>대시보드의 달력</B>에서
              합니다.
            </p>
          </Section>
          ...
          <Section title="목록에 있는 '전체 관리자'는 무엇인가요">
            <p>
              대원 목록에 <Badge className="bg-indigo-100 text-indigo-800">전체 관리자</Badge>{' '}
              배지가 붙은 계정이 하나 있습니다. <B>부대원이 아니라 이 앱을 만들고 관리하는
              사람의 계정</B>입니다. 서무와 같은 권한으로 앱 전체를 점검하지만, 서무 계정으로
              함께 들어오면 서로 화면을 건드리게 되어 따로 두었습니다.
            </p>
            <p className="mt-2">
              그래서 이 줄에는 <B>수정·비밀번호 초기화·삭제 버튼이 없습니다.</B> 실수로 지워지면
              되돌릴 방법이 없기 때문입니다. 부대원이 아니므로 <B>대리 입력 대상이나 사용 현황
              순위에도 나오지 않고,</B> 서무 수에도 포함되지 않습니다 — 이 계정이 있어도 부대에
              서무는 반드시 한 명 있어야 합니다.
            </p>
            <p className="mt-2">
              <B>서무가 따로 할 일은 없습니다.</B> 그냥 두시면 됩니다.
            </p>
          </Section>
          ...
          <Section title="일근인 날은 정원이 늘어납니다">
            <p>
              우리 부대는 <B>일근 → 당직 → 비번</B>이 3일마다 돌아갑니다. 달력에서 우리가 일근에
              걸리는 날은 날짜 옆에 작게 <B>일</B>이 붙습니다. 당직·비번인 날에는 아무 표시도 하지
              않습니다.
            </p>
            <p className="mt-2">
              일근인 날은 부대에 사람이 더 남아 있으므로 연가를 한 명 더 보낼 수 있습니다. 그래서 그
              날만 정원 설정의 <B>일근일 추가</B>만큼 <B>가능인원과 총 한도가 함께 올라갑니다.</B>{' '}
              예를 들어 가능인원 2명 · 총 한도 3명 설정에 일근일 추가가 +1명이면, 일근인 날은{' '}
              <B>가능인원 3명 · 총 한도 4명</B>이 됩니다. 달력 색과 남은인원도 이 늘어난 값으로
              계산됩니다.
            </p>
            <p className="mt-2">
              늘리지 않으려면 정원 설정에서 <B>일근일 추가를 +0명</B>으로 두면 됩니다. 기간마다 따로
              정할 수 있습니다.
            </p>
            <p className="mt-2">
              <B>근무 주기가 실제와 어긋난다면</B>(부대 사정으로 하루 밀리는 경우) 이 화면에서는 고칠
              수 없습니다. 앱을 만든 사람에게 알려주세요 — 기준이 되는 날짜 한 줄만 고치면 전체가 다시
              맞습니다.
            </p>
          </Section>
          ...
          <Section title="달력에서 서무가 할 수 있는 일">
            <ul className="space-y-1 list-disc list-inside">
              ...
              <li>
                ...
                우선인지 표시하는 용도입니다. <B>같은 날에 같은 번호를 두 사람에게 줄 수는
                없습니다</B> — 이미 쓰고 있는 번호를 고르면 누가 갖고 있는지 알려주고 저장하지
                않습니다. 순서를 바꾸려면 <B>한 사람의 순번을 먼저 해제</B>한 뒤 지정하세요.
              </li>
              <li>
          ...
          <Section title="사용 현황 — 누가 얼마나 썼는지 보기">
            <p>
              <B>사용 현황</B> 탭에서 그 달에 연가를 쓴 <B>대원 전원</B>을 많이 쓴 순서대로 볼 수
              있습니다. 유형별 일수와 합계가 함께 나오고, <B>◀ ▶</B> 로 달을 넘기면 지난달도 볼 수
              있습니다.
            </p>
            <p className="mt-2">
          ...
          <Section title="대리 입력 — 대원 대신 넣고, 고치고, 무르기">
            <p>
              앱을 쓰지 않는 대원의 연가도 달력에 들어와 있어야 그날의 가능인원이 실제와 맞습니다.{' '}
              <B>대리 입력</B> 탭에서 <B>대상 대원</B>을 고르면 그 사람의 신청이 모두 나오고, 거기서
              세 가지를 할 수 있습니다.
            </p>
            <ul className="mt-2 space-y-1 list-disc list-inside">
              <li>
                <B>새로 신청</B> — 대원이 직접 낸 것과 똑같이 달력·가능인원에 반영됩니다.
              </li>
              <li>
                <Badge className="bg-blue-100 text-blue-700">✏️ 파랑</Badge> 이미 들어온 신청의
                날짜·유형·추가 사항을 <B>고칩니다.</B> 취소하고 다시 넣을 필요가 없습니다.
              </li>
              <li>
                <Badge className="bg-red-100 text-red-700">🗑️ 빨강</Badge> 신청을{' '}
                <B>통째로 취소합니다.</B> 취소 사유를 반드시 입력해야 하고, 그 사유는 대원의{' '}
                <B>내 신청 내역</B>에 그대로 보입니다.
              </li>
            </ul>
            <p className="mt-2">
              <B>고치기와 통째 취소는 서무만</B> 할 수 있습니다. 대원은 지금처럼 신청과 취소만 하고,
              고치려면 취소한 뒤 다시 신청합니다.
            </p>
            <Callout tone="warn">
              <B>대원 화면에는 서무가 대신 넣었다는 표시가 남지 않습니다.</B> 본인이 낸 것과 똑같이
              보이므로, 대신 넣어준 뒤에는 <B>본인에게도 한마디 알려주세요.</B> 모르는 연가가 잡혀
              있으면 대원 쪽에서는 확인할 방법이 없습니다.
            </Callout>
            <Callout tone="warn">
              날짜를 옮기거나 기간을 줄이면 <B>신청에서 빠진 날짜의 순번은 함께 지워집니다.</B>{' '}
              그대로 두면 신청하지도 않은 날에 순위가 남아, 나중에 그 날짜로 신청할 때 먼저 신청한
              사람보다 앞에 서게 되기 때문입니다. 남은 날짜의 순번은 그대로 유지됩니다.
            </Callout>
            <p className="mt-2">
              <B>달력에서 하는 취소와는 범위가 다릅니다.</B> 달력에서는 여러 날 중{' '}
              <B>하루만</B> 뺄 수 있고, 여기서는 신청 <B>한 건 전체</B>가 취소됩니다. 하루만 빼려면
              달력에서 그 날짜를 눌러 취소하세요.
            </p>
          </Section>
